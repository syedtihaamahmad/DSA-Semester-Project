...
void guesslet (char a)
{
    ...
    if ((tried!=1) && (validlet==1)) //Check if letter is valid and hasn't been tried before
    {
        for (k=0;k<string(word).size();k++)
            if (wordRope->ElementAtIndex(wordRope,k)==a) //Check if the letter is in the word
            {
...
void selectRandLine ()
{
    ...
    char char_array[wordlength+1]; 
    
    // copying the contents of the 
    // string to char array
    strcpy(char_array, word.c_str()); 
      
    wordRope->create(wordRope, NULL, char_array, 0, wordlength-1);
}
